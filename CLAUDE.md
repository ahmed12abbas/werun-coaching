...
- **Who takes a session** is `coach_id` on two rows: `schedule` (who usually has the slot) and `club_sessions` (who had it that morning). A published session that names somebody is answered by that name **or by nothing** — it must not fall through to the slot's coach, or a stand-in's session gets attributed to the club's usual Tuesday coach. Publishing writes the slot's coach into the row when the form is left blank, so the fallback in `coachNameFor()` is only for the sessions that went out before any of this existed. Nothing joins `users` to get the name: `coach_id` arrives in a hand-applied migration, and a `JOIN` on a column the database has not got fails outright where `SELECT s.*` just omits it — so the id rides along and `lib/coaches.js` supplies the names. `coachRoster()` (no email, capped) is the one that runs on `/api/week`; `coachList()` is the console's.
