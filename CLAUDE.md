...
- **When check-in is open is worked out, never read back.** `windowFor()` in `lib/checkin.js` takes a session's `starts_at` and the club's two settings (open a month before, shut two hours after) and every decision goes through it — the checkin route, `/api/session`, the week, the console's list and its QR screen. `window_open_at`/`window_close_at` on the row are still written and still true, but they are only what the row said when it went out: reading them back would leave every session already on the calendar under whatever window it was published with, so widening the window in the console would reach nothing. It is also why the club's numbers are what they are — an early athlete is not the thing worth stopping, the thirty-second signature is, so the window is wide before and short after.
- **The line under Where is the slot's, not the session's.** `schedule.desc_en`/`desc_ar` — "Long run 80min" — written once in the console and inherited by every session published into that slot (`publishedItem` in `lib/weekplan.js`, and the join in `routes/sessions.js`). A save that never mentions `desc_en` leaves it alone, because `tools/seed-schedule.js` rewrites all ten rows and must not take the coach's own lines with it.
- **`/coach` is bilingual, `/admin` is not.** The track screen carries the same two toggles the athlete app does — language and light/dark — and every string on it, including the ones in `js/console.js`, goes through `t()` under a `c*` prefix. `/admin` sets `I18N.lang = "en"` directly (not `I18N.apply()`, which would write the choice to localStorage and change the language the athlete app opens in) because its own two thousand lines are English and half a translated page is worse than none. `consoleTools()` in `js/console.js` draws the toggles with the console's own `el()`; `js/brand.js` is not loaded, because its version wants the app's `el()` and its `html` attribute. `coach.html` draws its own heading and footer so both can be redrawn in the other language, and `LAST` holds the four payloads so a toggle redraws rather than refetches. Day names come from `toLocaleDateString`, not a table of seven strings.
- **Two coach pages, and a real boundary between them.** `/coach` is the track screen — the head count, the code for every session this week, and who scanned it, and nothing that edits. `/admin` is everything else. They share `assets/console.css` and `js/console.js` (login, `api()`, `el()`, `qrScreen()`, `attendanceCards()`, `consoleBoot()`), because there is no bundler and a second copy is a second thing to keep in step; each page defines its own `load(pw)`, which is the only thing `renderLogin()` and `consoleBoot()` need from it. `consoleBoot()` tries the cookie before drawing the form, so a coach who tapped through from the app's Me screen is already in.
- **Coach and admin are two columns, not one ladder.** `users.role` is who takes the sessions; `users.is_admin` (migration 0010) is who runs the club, and the head coach is both. Admin as a third `role` value would drop an admin out of `coachRoster()`, the roster and the week the first time a query forgot `role IN ('coach','admin')` — and `role` carries a CHECK constraint SQLite cannot alter, so it would have meant rebuilding `users` by hand in the D1 console. `refuseUnlessCoach()` is the track guard (stats, qr, and `list`/`roster`/`open` on sessions, `list` on schedule); `refuseUnlessAdmin()` is everything that changes the club. Until 0010 is applied `hasColumn()` makes every coach an admin, exactly as before — `memberList()` fakes the column the same way so the screen and the guard cannot disagree. `ADMIN_PASSWORD` still opens both, because it makes the first admin and is the way back — `refuseUnlessAdmin()` tries a password that was actually sent *before* it refuses a coach, or a coach cookie would be what breaks the documented way back in. `/tips` is deliberately still coach-reachable (and admin-reachable — `isStaff()` in `routes/tips.js`): writing articles is not running the club.
- **The rota is the coaches' own, and nothing else's.** `coach_rota` (migration 0011) is a coach putting *themselves* down for a standing slot on a date — many coaches per session, ahead of the week, so the club can see a Wednesday nobody is on. Keyed on `(schedule_id, date, user_id)`, not on a session id: seven of the ten slots have no `club_sessions` row until somebody shows a code, and making one to record a tick would put empty sessions on the calendar weeks ahead. It is emphatically **not** `coach_id` — that is one name, on a published workout, shown to athletes; this is many, on a slot, shown to nobody but coaches. `/api/coach/rota` is coach-tier; `join` needs `isCoach()` specifically (the club password is nobody in particular and cannot say who it is putting down), and `leave` on somebody else's row needs an admin. `adminHere()` there compares the club password itself rather than calling `refuseUnlessAdmin()`, because `refuseUnlessCoach()` has already charged the rate limit for this request.
- The coach tier is reads apart from `action: "open"` and the rota, which writes the session row a code is signed against — find-or-create, and bounded to ±60 days so the calendar cannot be filled with sessions nobody is standing at. `roster` on that tier selects `u.name` and not `u.email`: walking every session id would otherwise be the whole membership's addresses, and the CSVs in `routes/export.js` are the admin-only way to those.
- Nobody can take their own `is_admin` off (`not-yourself` on `/api/admin/members`) — it is the one mis-tap that locks you out of the screen you are standing on. `admin.html`'s `load()` asks `/api/admin/settings` before it draws anything, so a coach meets the door rather than thirteen cards each refusing them separately.
- **The dashboard counts check-ins, not share taps.** `/api/stats` reads `club_sessions` with a live-check-in count per row and gathers them into the club's Sunday-start weeks (`clubWeekStart()` in `lib/week.js`). The old KV share counter — `POST /api/share`, `DOC_KEY`, `countShare()` in `js/model.js` — is gone: it said how far a link travelled and nothing about who ran. A voided check-in stops counting, which is why the query filters `voided_at IS NULL` rather than trusting a stored total.
...
- **Who takes a session** is `coach_id` on two rows: `schedule` (who usually has the slot) and `club_sessions` (who had it that morning). A published session that names somebody is answered by that name **or by nothing** — it must not fall through to the slot's coach, or a stand-in's session gets attributed to the club's usual Tuesday coach. Publishing writes the slot's coach into the row when the form is left blank, so the fallback in `coachNameFor()` is only for the sessions that went out before any of this existed. Nothing joins `users` to get the name — a week is seven days of rows and joining each to the members table for four names is the wrong shape, so the id rides along and `lib/coaches.js` supplies the names from one small read. `coachRoster()` (no email, capped) is the one that runs on `/api/week`; `coachList()` is the console's.
- Coaches are `users.role`, read two ways: the members table toggles one person, `/api/admin/coaches` is the short list plus everyone who could join it. A coach cannot take *themselves* off — the club password is the way back, but not from the screen they are standing on.
...
- The twelve avatars are **drawn**, not downloaded (`js/avatars.js`): one SVG string each. The animals bound and the cars roll, from one block of keyframes in `assets/app.css`; **the six people hold still** at one stride, because at badge size a person's swinging arm reads as a sprawl where a four-legged bound and a turning wheel still read. The id list is duplicated in `_worker.js/routes/auth.js` (`AVATARS`) because there is no bundler to share one copy — a new avatar goes in both or the Worker refuses it. An unknown id becomes `""`, which is the member's initial, not an error.
- The profile form asks for an **age**; the database keeps the **birth year** (`js/app.js`, `yearOfAge`/`ageOf`). An age written down is wrong from the next birthday and nobody comes back to fix it, so the two convert at that one edge.
