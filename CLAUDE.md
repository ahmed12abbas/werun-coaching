...
- **Two coach pages, one set of keys.** `/coach` is the track screen — the head count, the code for every session this week, and who scanned it, and nothing that edits. `/admin` is everything else. They share `assets/console.css` and `js/console.js` (login, `api()`, `el()`, `qrScreen()`, `attendanceCards()`), because there is no bundler and a second copy is a second thing to keep in step; each page defines its own `load(pw)`, which is the only thing `renderLogin()` needs from it. The split is which screen fits the moment and **not** a permission boundary: both call the same routes and every coach opens both. A real boundary needs a second role on `users.role`.
