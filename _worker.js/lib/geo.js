...
export async function pinFor(env, url) {
  ...
  // Only a cache: a full KV quota means asking Google again next time, not a refused coach.
  if (env.STATS) await env.STATS.put(key, pin ? JSON.stringify(pin) : "none", { expirationTtl: KEEP_S }).catch(() => {});
  return pin;
}
